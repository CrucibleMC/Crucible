...
public class SushchestvoCommand extends Command {
    ...
    public boolean execute(CommandSender sender, String commandLabel, String[] args) {
        ...
        if ((args.length == 1) && "save".equalsIgnoreCase(args[0])) {
            MinecraftServer.sushchestvoConfig.save();
            sender.sendMessage(ChatColor.GREEN + "Config file saved");
            return true;
        }
        if ((args.length == 1) && "reload".equalsIgnoreCase(args[0])) {
            MinecraftServer.sushchestvoConfig.load();
    ...
    private boolean getToggle(CommandSender sender, String[] args) {
        try {
            Setting toggle = MinecraftServer.sushchestvoConfig.getSettings().get(args[1]);
            // check config directly
            if (toggle == null && MinecraftServer.sushchestvoConfig.isSet(args[1])) {
                if (MinecraftServer.sushchestvoConfig.isBoolean(args[1])) {
                    toggle = new BoolSetting(MinecraftServer.sushchestvoConfig, args[1], MinecraftServer.sushchestvoConfig.getBoolean(args[1], false), "");
                } else if (MinecraftServer.sushchestvoConfig.isInt(args[1])) {
                    toggle = new IntSetting(MinecraftServer.sushchestvoConfig, args[1], MinecraftServer.sushchestvoConfig.getInt(args[1], 1), "");
                }
                if (toggle != null) {
                    MinecraftServer.sushchestvoConfig.getSettings().put(toggle.path, toggle);
                    MinecraftServer.sushchestvoConfig.load();
                }
            }
            if (toggle == null) {
    ...
    private boolean intervalSet(CommandSender sender, String[] args) {
        try {
            ...
            MinecraftServer.sushchestvoConfig.set(args[1], setting);
        } catch (Exception ex) {
            sender.sendMessage(ChatColor.RED + "Usage: " + usageMessage);
            return false;
        }
    ...
    public List<String> tabComplete(CommandSender sender, String alias, String[] args) {
        ...
        if (((args.length == 2) && "get".equalsIgnoreCase(args[0])) || "set".equalsIgnoreCase(args[0])) {
            return StringUtil.copyPartialMatches(args[1], MinecraftServer.sushchestvoConfig.getSettings().keySet(), new ArrayList<String>(MinecraftServer.sushchestvoConfig.getSettings().size()));
        }
