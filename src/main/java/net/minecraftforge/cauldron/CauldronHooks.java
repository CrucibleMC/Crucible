...
//    }
    ...
    public static byte canSushchestvoTick(Entity entity, World world) {
        if (entity == null || world.sushchestvoConfig == null) {
            return 0;
        }
        if (!MinecraftServer.sushchestvoConfig.skipEntityTicks.getValue()) {
            return 1;
        }
    ...
    public static boolean canUpdate(TileEntity tileEntity) {
        return tileEntity != null && tileEntity.canUpdate() && !MinecraftServer.bannedTileEntityUpdates.contains(tileEntity.getClass()); // quick exit
    }
