    ...
    private NBTBase readListTag() throws NBTException
    {
        ...
        else
        {
            ...
            while (this.peek() != ']')
            {
                ...
                {
                    throw this.exception("Unable to insert " + NBTBase.NBTTypes[j] + " into ListTag of type " + NBTBase.NBTTypes[i]);
                }
    ...
    private <T extends Number> List<T> readArray(byte p_193603_1_, byte p_193603_2_) throws NBTException
    {
        ...
        while (true)
        {
            if (this.peek() != ']')
            {
                ...
                if (i != p_193603_2_)
                {
                    throw this.exception("Unable to insert " + NBTBase.NBTTypes[i] + " into " + NBTBase.NBTTypes[p_193603_1_]);
                }

                if (p_193603_2_ == 1)
                {
                    list.add((T)Byte.valueOf(((NBTBase.NBTPrimitive)nbtbase).func_150290_f()));
                }
                else if (p_193603_2_ == 4)
                {
                    list.add((T)Long.valueOf(((NBTBase.NBTPrimitive)nbtbase).func_150291_c()));
                }
                else
                {
                    list.add((T)Integer.valueOf(((NBTBase.NBTPrimitive)nbtbase).func_150287_d()));
                }
